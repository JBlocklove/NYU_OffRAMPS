...
<!--
    ...
    * Components are sorted by ref and grouped by same value+footprint
    One value per line
    Fields are
    Comment,Designator,Footprint,Vendor
    The command line is
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/export">
	    <xsl:text>Comment,Designator,Footprint,Vendor</xsl:text>
	    <!-- all table entries -->
	...
	<xsl:template match="components">
		...
		<xsl:value-of select="fields/field[@name='Vendor']"/><xsl:text>"</xsl:text>
	    </xsl:for-each>
	</xsl:template>
